...
class BEVFusion(Base3DFusionModel):
    def __init__(
        ...
        **kwargs,
        ...
        if fuser is not None:
            ...
            # self.input_self_transformer = TransformerEncoder(num_layers=3,input_dim=256,dim_feedforward=128,num_heads=4,cross_label=False,bev_cross_label=False)
            # self.input_cross_transformer = TransformerEncoder(num_layers=1, input_dim=256,dim_feedforward=128, num_heads=4,cross_label=True,bev_cross_label=True)
            #
            # self.output_self_transformer = TransformerEncoder(num_layers=1,input_dim=320,dim_feedforward=128,num_heads=2,cross_label=False,bev_cross_label=False)
            # self.output_cross_transformer = TransformerEncoder(num_layers=1,input_dim=320,dim_feedforward=128,num_heads=2,cross_label=True,bev_cross_label=False)
        ...
        #
        # self.mask_map = Spatial_info.EdgeDetectionNet()
    ...
    def extract_camera_features(
        ...
        # # Add mask map information
        # for i in range(len(x)):
        #     output = self.input_self_transformer(x,feat_tag=i,pe=img_pe,mask_map=mask_map)
        #     transformer_features.append(output)
        # transformer_features_tensor = torch.cat(transformer_features,dim=0)
        # # Cross-attention here
        # for i in range(len(transformer_features_tensor)):
        #     output = self.input_cross_transformer(transformer_features_tensor,feat_tag=i, pe=img_pe,mask_map=mask_map)
        #     final_features.append(output)
        #
        # x = torch.cat(final_features,dim=0)
    ...
    def forward_single(
        ...
        **kwargs,
        ...
        for sensor in (
            ...
            if sensor == "camera":
                ...
                if 'infra' in list(self.encoders.keys()):
                    ...
                    point_cloud_concat = torch.concat((points[0], torch.from_numpy(infra_point_clouds).cuda().to(dtype=torch.float16)),dim=0)
                    torch.cuda.empty_cache()
                    
                    feature = self.extract_features([point_cloud_concat], sensor)
                else:
        ...
        if self.fuser is not None:
            ...
            # for i in range(len(transformer_features)):
            #     output = self.output_cross_transformer(transformer_features,feat_tag=i, pe=bev_pe_list)
            #     final_features.append(output)
            #
